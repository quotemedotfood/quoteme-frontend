...
import { DistributorMemoryLabel } from './DistributorMemoryBadge';
...
const CORRECTION_TYPE_LABELS: Record<CorrectionType, string> = {
  ...
  distributor_mandate: 'Distributor mandate (contract/compliance)',
};
...
export function MatchDrawer({
  ...
  // Ruling 2: a distributor mandate MUST be attributable -- the BE 422s a
  // distributor_mandate submission with no reason, so the field is required
  // in this UI whenever that reason is selected.
  ...
  const handleSubmit = async () => {
    ...
    // Ruling 2: a mandate MUST be attributable -- block the submit client-side
    // (the BE also 422s) rather than send a mandate with no reason.
    if (correctionType === 'distributor_mandate' && !mandateReason.trim()) {
      setSubmitError('A reason is required for a distributor mandate.');
      return;
    }
    setSubmitting(true);
    setSubmitError(null);
    ...
    const res = await submitYourCallSelection(quoteId, {
      ...
      mandate_reason: correctionType === 'distributor_mandate' ? mandateReason.trim() : null,
    });
    setSubmitting(false);
    if (res.error) {
        ...
        <div className="qm-match-drawer-scroll flex-1 overflow-y-auto px-[22px] pt-[18px] pb-[10px] min-h-0">
                    ...
                    <h4 className="text-[13.5px] font-semibold leading-[1.3] flex items-center gap-[6px]" style={{ color: 'var(--qm-charcoal)' }}>
                      ...
                      {!currentMatch.rep_memory && currentMatch.distributor_memory && (
                        <DistributorMemoryLabel
                          signalType={currentMatch.distributor_signal_type === 'mandate' ? 'mandate' : 'preference'}
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-[13.5px] font-semibold leading-[1.3] flex items-center gap-[6px]" style={{ color: 'var(--qm-charcoal)' }}>
                          ...
                          {!candidate.rep_memory && candidate.distributor_memory && (
                            <DistributorMemoryLabel
                              signalType={candidate.distributor_signal_type === 'mandate' ? 'mandate' : 'preference'}
          ...
          {canReplace && (
            <div className="mt-[18px]">
              ...
              {/* Ruling 2: a mandate MUST be attributable -- required whenever
                  "Distributor mandate" is the selected reason. */}
              {correctionType === 'distributor_mandate' && (
                <input
                  type="text"
                  value={mandateReason}
                  onChange={(e) => setMandateReason(e.target.value)}
                  placeholder="Why is this a mandate? (e.g. contract transition to new supplier)"
                  className="w-full mt-2 rounded-[8px] px-3 py-[10px] text-[13px] outline-none"
                  style={{ border: '1px solid var(--qm-soft-line)', fontFamily: 'var(--qm-sans)', color: 'var(--qm-charcoal)', background: '#fff' }}
                />
              )}
            </div>
